...
describe('user permissions', () => {
  ...
  it('keeps role field editable only for administrators in edit mode', () => {
    ...
    expect(getFieldRule(roleField, ROLES.ADMIN, MODES.EDIT)).toBe(FIELD_RULES.EDITABLE)
    expect(getFieldRule(roleField, ROLES.STUDENT, MODES.EDIT)).toBe(FIELD_RULES.HIDDEN)
  })
  ...
  it('returns user-friendly required errors', () => {
    ...
    expect(
      result.error.issues.some(
        (issue) => issue.path[0] === 'email' && issue.message === 'Email is required',
      ),
    ).toBe(true)
  })
})
