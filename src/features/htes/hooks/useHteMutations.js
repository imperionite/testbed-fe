...
export function useHteMutations() {
  ...
  const invalidateHtes = () => queryClient.invalidateQueries({ queryKey: ['htes'] })
  ...
  const updateStatus = useMutation({
    mutationFn: ({ id, isActive }) => htesApi.updateStatus(id, { isActive }),
    onSuccess: invalidateHtes,
  })

  const updateHteSupervisor = useMutation({
    mutationFn: ({ id, supervisorId }) => htesApi.updateHteSupervisor(id, { supervisorId }),
    onSuccess: invalidateHtes,
  })

  const bulkUpdateStatus = useMutation({
    mutationFn: ({ ids, isActive }) =>
      Promise.all(ids.map((id) => htesApi.updateStatus(id, { isActive }))),
    onSuccess: invalidateHtes,
  })
