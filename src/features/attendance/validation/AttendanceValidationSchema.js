...
const updateAttendanceSchema = z.object({
  ...
  time_in: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, "Invalid time format (HH:mm)").optional(),
  time_out: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, "Invalid time format (HH:mm)").optional(),
}).refine(
  (data) => data.attendance_date !== undefined || data.time_in !== undefined || data.time_out !== undefined,
