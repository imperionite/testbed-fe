...
import { Box, Typography, Stack, CircularProgress, Alert, TextField, Dialog, DialogTitle, DialogContent, DialogActions, Button } from "@mui/material";
...
export default function CoordinatorDocumentsView({ internshipId }) {
  ...
  const handleRejectConfirm = async () => {
    ...
    await rejectDocument.mutateAsync({ documentId: rejectDialog.documentId, reason: rejectionReason });
  ...
  return (
    <Box>
      <Typography variant="h6" gutterBottom>Internship Documents</Typography>
      <Stack spacing={1}>
      ...
      <Dialog open={rejectDialog.open} onClose={() => setRejectDialog({ open: false, documentId: null })}>
        ...
        <DialogActions>
            ...
            <Button onClick={handleRejectConfirm} variant="contained" color="error" disabled={!rejectionReason}>Reject</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
